...
export class TableDialogComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  exportToPDF() {
    ...
    columns.forEach((header, i) => {
      doc.text(header, margin + i * columnWidth + 2, y + 7, { maxWidth: columnWidth - 4 });
    });
    y += rowHeight;
    ...
    tableData.forEach((row, rowIndex) => {
      if (y > pageHeight - 20) {
        addFooter(pageNum, Math.ceil(tableData.length / ((pageHeight - startY) / rowHeight)));
        doc.addPage();
        pageNum++;
        y = startY;
      ...
      columns.forEach((col, i) => {
        ...
        doc.text(cellContent, margin + i * columnWidth + 2, y + 7, { maxWidth: columnWidth - 4 });
      });
      y += rowHeight;
    });
